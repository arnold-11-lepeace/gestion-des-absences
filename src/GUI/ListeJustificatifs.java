...
public class ListeJustificatifs {
	...
	private JFrame listeJustifFrame;
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					...
					window.listeJustifFrame.setVisible(true);
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
	...
	public ListeJustificatifs() {
		initialize();
		listeJustifFrame.setVisible(true);
	}
	...
	private void initialize() {
		listeJustifFrame = new JFrame();
		listeJustifFrame.setBounds(100, 100, 450, 300);
		listeJustifFrame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		...
		btnDeconnexion.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				listeJustifFrame.setVisible(false);
			...
			for (int col = 0; col < 5; col++) {
				if (row == 0) {
					...
					case 0:
						...
						break;
					case 1:
						JButton btnNomFichier = new JButton(
								"" + listJustificaifEleve.get(row).getNomFichierJustificatif());
		...
		for (int i = 0; i < listBtnNomFichier.size(); i++) {
			...
			listBtnNomFichier.get(i).addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					/*
					 * // TODO Auto-generated method stub try { //Créer une instance PdfReader.
					 * PdfReader pdf = new PdfReader("C:\\my.pdf");
					 * 
					 * //Récupérer le nombre de pages en pdf. int nbrPages = pdf.getNumberOfPages();
					 * 
					 * //Itérer le pdf à travers les pages. for(int i=1; i <= nbrPages; i++) {
					 * //Extraire le contenu de la page à l'aide de PdfTextExtractor. String content
					 * = PdfTextExtractor.getTextFromPage(pdf, i);
					 * 
					 * //Afficher le contenu de la page sur la console.
					 * System.out.println("Contenu du page : " + content); }
					 * 
					 * //Fermez le PdfReader. pdf.close();
					 * 
		...
		listeJustifFrame.setContentPane(panelBase);
	}
